...
| **clipboard / location** | clipboard=**user-event 설치 스텁에 `vi.spyOn`** (Do 중 수정: `userEvent.setup()`이 자체 동작하는 clipboard를 설치하므로 `vi.stubGlobal`은 무효 — 실측), location=**`navigate` prop 주입** (QuoteForm에 `navigate?: (href: string) => void` 추가, 기본값 `window.location.href` 할당) | jsdom은 clipboard 미구현·href 할당 시 "Not implemented: navigation" 가상콘솔 에러. prop 주입이 유일하게 결정적(스파이 가능)이며 프로덕션 동작 불변 |
...
| `src/test-setup.ts` | `@testing-library/jest-dom/vitest` 매처 + **jsdom 미구현 폴리필**: `Element.prototype.scrollIntoView` 스텁 (QuoteForm 포커스 이동에서 사용, Do 중 실측 추가) |
...
| `ContactActions.render.test.tsx` | quote 액션=`/quote` 링크(동시 작업 커밋 `9fdb064`로 mailto→페이지 링크 변경됨), partner/schedule=mailto href(subject 인코딩), scheduleUrl 제공 시 `target="_blank"`+`rel="noopener noreferrer"` |
...
| 9 | share-card/twitter (seo.ts·service-pages.ts 문자열) | **데이터 검증 대체** | `toMatchObject({ card: 'summary_large_image', images: [shareImage] })` — Next `Twitter` 유니언 타입이 `.card` 직접 접근을 거부해 toMatchObject 사용 (Do 중 실측) |
