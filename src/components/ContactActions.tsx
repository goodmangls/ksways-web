...
type Intent = 'quote' | 'partner' | 'schedule';

function buildMailto(email: string, locale: Props['locale'], intent: Intent) {
  const subjectMap: Record<Props['locale'], Record<Intent, string>> = {
    en: {
  ...
  const bodyMap: Record<Props['locale'], Record<Intent, string>> = {
    en: {
      quote:
  ...
  return `mailto:${email}?subject=${encodeURIComponent(subjectMap[locale][intent])}&body=${encodeURIComponent(bodyMap[locale][intent])}`;
}
